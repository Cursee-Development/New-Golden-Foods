...
public abstract class NeoForgeFishingHookMixin {
    ...
    private boolean redirected$retrieveAddFreshEntity0(Level instance, Entity entity) {
        ...
        if (itemEntity.getItem().is(Items.COD)) {
            return instance.addFreshEntity(new ItemEntity(instance, player.getX(), player.getY(), player.getZ(), new ItemStack(ModItemsNeoForge.ENCHANTED_GOLDEN_COOKED_COD.get())));
        }
        else if (itemEntity.getItem().is(Items.SALMON)) {
            return instance.addFreshEntity(new ItemEntity(instance, player.getX(), player.getY(), player.getZ(), new ItemStack(ModItemsNeoForge.ENCHANTED_GOLDEN_COOKED_SALMON.get())));
        }
